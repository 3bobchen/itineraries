...
The color stance is **Committed**: one warm campari-crimson carries 30–60% of key surfaces — day markers, section headers, hero blocks, the structural spine of an itinerary — rather than hiding as a 10% accent. Identity comes from that single confident color plus typographic order, never from decoration. The system explicitly rejects what PRODUCT.md rejects: SEO travel-blog listicle sludge, Expedia/Booking.com conversion pressure, and the generic AI/SaaS look (cream backgrounds, gradient text, icon-card grids, eyebrow labels).

**Key Characteristics:**
- Committed single-color identity: crimson as structure, not garnish
...
A Committed strategy built around one warm crimson; everything else stays disciplined neutral so the color reads as identity, not noise.

### Primary
- **Campari Crimson** (anchor hue ~20° in OKLCH, near oklch(0.55 0.18 20); exact ramp [to be resolved during implementation]): carries day markers, itinerary spines, primary actions, active states, and key headers. White text on crimson fills, always.
...
### Named Rules
**The Committed Crimson Rule.** The crimson is structural — it marks days, anchors headers, and signals actions. It covers 30–60% of identity surfaces and 0% of body prose. If a screen has crimson decoration that carries no meaning, remove it.

**The No-Hidden-Warmth Rule.** Warmth lives in the crimson and the voice, never in the background. The surface is pure white (or pure near-black), chroma zero. Cream/sand/beige backgrounds are prohibited.
...
### Do:
- **Do** use the crimson structurally (day markers, headers, primary actions) at Committed scale — 30–60% of identity surfaces.
